...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if len(self.getLegalActions(state)) == 0:
            ...
            for action in self.getLegalActions(state):
                maxActionQ = max(maxActionQ, self.getQValue(state, action))
        return maxActionQ
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.Q[(state, action)] = (1-self.alpha)* self.getQValue(state, action)+ self.alpha* ((self.discount*self.computeValueFromQValues(nextState)) + reward )
